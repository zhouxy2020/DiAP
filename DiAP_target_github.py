...
MODEL_NAMES = [ 'inceptionv3',#'vgg16','vgg19','xception','resnet50'
                ]
...
def test_random_transform_backup(img, min_scale=0.1, max_scale=1.0, max_rotation=22.5):
	"""
	Scales the image between min_scale and max_scale
	"""
	sess = tf.compat.v1.Session()
	image_in = tf.compat.v1.placeholder(dtype=tf.float32, shape=img.shape)
	width = img.shape[0]
	...
	def _random_transformation():
		...
		# x_delta = padding_after_scaling
		# y_delta = padding_after_scaling
	...
	# padded_patch = tf.contrib.image.transform(patch, random_xform_vector, "BILINEAR")

	xformed_img = sess.run(output, feed_dict={image_in: img})
	return xformed_img


def test_random_transform(img, min_scale=0.1, max_scale=1.0, max_rotation=22.5):
	"""
	Scales the image between min_scale and max_scale
	"""
	sess = tf.compat.v1.Session()
...
class ModelContainer():
	...
	def train_step(self, images=None, target_ys=None, learning_rate=5.0, scale=(0.1, 1.0),
	               ...
	               dropout=None, patch_disguise=None, disguise_alpha=None):
		...
		if images is None:
			...
			for _ in range(BATCH_SIZE):
				im = PIL.Image.open('./content/DiAP/' + self.model_name + '.jpg')
				r = np.random.randint(0, 360)
				w = np.random.randint(50, 298)
				im = im.rotate(r)
				im = im.resize((w, w), PIL.Image.ANTIALIAS)
				im = np.array(im.getdata()).reshape(w, w, 3)/128. -1
				im = im * (_circle_mask(im.shape) != 0)
				s = np.random.randint(0, 299 - w)
				temp = np.zeros((299, 299, 3))
				temp[s:s+w, s:s+w, :] = im
				images.append(temp)
	...
	def _make_model_and_ops(self, M, keras_mode, patch_val, verbose):
		...
		with self.sess.graph.as_default():
			...
			if self.peace_mask == 'forward':
				...
				modified_patch = patch * (1 - mask) - np.ones(self.patch_shape) * mask + (
			...
			self.ouput_layer = [layer.output for layer in model.layers]
	...
	def train_step(self, model=None, steps=1, images=None, target_ys=None, learning_rate=5.0, par_loss=[1.,0.], scale=None, **kwargs):
		...
		for mc in to_train:
			...
			for i in range(steps):
				images=[]
				for _ in range(BATCH_SIZE):
					im = PIL.Image.open('./content/DiAP/' + transfer_ensemble_model_names[np.random.choice(4)] + '.jpg')
					r = np.random.randint(0, 360)
					w = np.random.randint(50, 298)
					im = im.rotate(r)
					im = im.resize((w, w), PIL.Image.ANTIALIAS)
				...
				loss = mc.train_step(images, target_ys, learning_rate, scale=scale, par_loss=par_loss, **kwargs)
				losses.append(loss)
			self.patch(mc.patch())
...
img_list_test = './utils/ilsvrc_1000_train.txt'
...
for m in model_targets:
	...
	for i in range(STEPS):
		x += 1
		loss = M.train_step(learning_rate=8.,par_loss=[0., 1.],)
		if i % int(STEPS / 10) == 0:
...
save_obj(regular_training_nontarget_rates, "regular_training_nontarget_rates")
save_obj(regular_training_target_rates, "regular_training_target_rates")

plot_win_rates(regular_training_nontarget_rates, model_targets, "Single-model nontarget attack success rates")
plot_win_rates(regular_training_target_rates, model_targets, "Single-model target attack success rates")

################## transfer attack
# LR = 8.0
# STEPS = 800
# transfer_ensemble_model_names = [  'inceptionv3', 'resnet50', 'vgg19', 'vgg16',]
# transfer_target_model ='xception'
#
...
# 		loss = MM.train_step(mm, steps=1, learning_rate=LR,par_loss=[0., 1.],)
# 	if i % int(STEPS / 10) == 0:
# 		print("[%s] loss: %s" % (i, loss))
#
# transfer_patch = MM.patch()
# save_obj(transfer_patch, "transfer_patch_xception")
#
# m = MM.nc[transfer_target_model]
# m.patch(transfer_patch)
#
# _ = report(m, show_images=3, scale=0.5)
#
# transfer_patch = load_obj("transfer_patch_xception")
#
# m = MM.nc[transfer_target_model]
# m.patch(transfer_patch)
#
...
#
# save_obj(transfer_attack_win_rates_nontarget, "transfer_attack_win_rates_xception_nontarget")
# save_obj(transfer_attack_win_rates_target, "transfer_attack_win_rates_xception_target")
#
# transfer_attack_win_rates_nontarget = load_obj("transfer_attack_win_rates_xception_nontarget")
# transfer_attack_win_rates_target = load_obj("transfer_attack_win_rates_xception_target")
# plot_win_rates(transfer_attack_win_rates_nontarget, ['xception - Blackbox transfer attack'], "blackbox model nontarget attack success rates")
# plot_win_rates(transfer_attack_win_rates_target, ['xception - Blackbox transfer attack'], "blackbox model target attack success rates")

# ################## ensemble attack
# LR = 8.0
# STEPS = 800
# transfer_ensemble_model_names = [ 'resnet50', 'xception',  'inceptionv3' , 'vgg16' ,'vgg19']
# # transfer_target_model = 'inceptionv3'
#
...
# 		print("[%s] loss: %s" % (i, loss))
#
# ensemble_target_patch = MM.patch()
# save_obj(ensemble_target_patch, "ensemble_target_patch")
# ensemble_target_patch = load_obj("ensemble_target_patch")
#
# models =[]
# for m in transfer_ensemble_model_names:
# 	M = MM.nc[m]
# 	M.patch(ensemble_target_patch)
# 	models.append(M)
#
...
#
# ensemble_attack_win_rates_nontarget = load_obj("ensemble_attack_win_rates_nontarget")
# ensemble_attack_win_rates_target = load_obj("ensemble_attack_win_rates_target")
# plot_win_rates(np.mean(ensemble_attack_win_rates_nontarget,axis=0)[np.newaxis,:], ['Ensemble attack'], "Ensemble-model nontarget attack success rates")
# plot_win_rates(np.mean(ensemble_attack_win_rates_target,axis=0)[np.newaxis,:], ['Ensemble attack'], "Ensemble-model target attack success rates")
#
# ################# Cross Model Attack
# STEPS = 1000
#
...
#
# toaster = load_image('/content/DiAP/toaster.png')
#
# models = []
# for m in MODEL_NAMES:
# 	M = MM.nc[m]
